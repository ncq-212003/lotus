...
export default function EditFormProcess({ openEditFormProcess, closeEditFormProcess, id }) {
    ...
    const selectRow = Array.isArray(processes[0]) ? processes[0].find(pro => pro.processId == id) : [];
        ...
        onSubmit: async (values, helpers) => {
            try {
                const formData = {
                    ...
                    Color: values.mauSac,
                    Icon: "1", // sau thay thanh link hinh anh
    ...
    useEffect(() => {
        const fetchProcessData = async () => {
            try {
                formik.setValues({
                    tieuDeTienTrinh: selectRow.processTitle || "",
                    tienDo: selectRow.processNumber || "",
                    mauSac: selectRow.color || "",
                    hinhAnh: selectRow.hinhAnh || "",
                    ghiChu: selectRow.description || "",
                })
            } catch (error) {
