...
export default function CompanyReceivingTable() {
    ...
    const dataWithSTT = Array.isArray(companies) ? companies.map((x, index) => ({
        ...x,
        stt: index + 1,
        id: x.id || index + 1,
    })) : [];
    ...
    const columns = [
        ...
        { field: 'licenseBusiness', headerName: 'Giấy phép kinh doanh', width: 120 },
        {
            ...
            headerName: 'Người đại diện (chức vụ)',
            width: 180,
            renderCell: (params) => (
                <span>{params.value + ' - ' + params.row.personResponsibiltyRole}</span>
            ),
        },
        ...
        { field: 'syndicateId', headerName: 'Nghiệp đoàn', width: 120 },
        {
            field: 'action',
            headerName: 'Thao tác',
            width: 150,
