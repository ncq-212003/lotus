...
export default function DepartmentEdit({ open, onClose, id }) {
  ...
  const companyName = companyNameOption.find(
    (x) => x.value === dataEdit?.companyId
  );

  const branchName = branchNameOption.find(
    (x) => x.value === dataEdit?.branchId
  );
    ...
    onSubmit: async (values, helpers) => {
      try {
        const formData = {
          ...
          BranchIdHidden: values.branch.value,
    ...
    const fetchCompanyData = async () => {
      try {
        ...
        formik.setValues({
          ...
          mainPersonInCharge: dataEdit.employeeIdMain,
            ...
            >
              <Grid container rowSpacing={1} columnSpacing={{ xs: 1, sm: 2 }}>
                ...
                <Grid item sm={12} xs={12} md={6}>
                  ...
                  <Autocomplete
                    onBlur={formik.handleBlur}
                    onChange={(event, newValue) => formik.setFieldValue("mainPersonInCharge", newValue)}
