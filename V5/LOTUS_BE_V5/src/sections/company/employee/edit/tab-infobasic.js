...
export function validateFieldInfobasic(dispatch, tab, fieldName, fieldValue) {
  const validationSchema = Yup.object().shape({
    ...
    department: Yup.array()
      .of(
        Yup.object().shape({
          value: Yup.number().required("Vui lòng nhập thông tin vào trường này2"),
          label: Yup.string().max(4000).required("Vui lòng nhập thông tin vào trường này4"),
        })
      )
      .min(1, "Vui lòng nhập thông tin vào trường này5"),
    role: Yup.array()
      .of(
        Yup.object().shape({
...
export default function InfoBaseEmployee() {
  ...
  const optionForRole = DepartmentRoleOption();
