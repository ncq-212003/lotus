...
import { findPersonBySyndicateIdApi, updatePersonApi } from "src/contexts/api/partner/api-person";
...
export default function ContactEdit({ open, onClose, rowData, syndicateId }) {
    const [state, dispatch] = useApp();
    const [snackbarOpen, setSnackbarOpen] = useState(false);
    const [snackbarMessage, setSnackbarMessage] = useState("");
    const [snackbarSeverity, setSnackbarSeverity] = useState("success");
    const [isLeaderSwitch, setIsLeaderSwitch] = useState((rowData && rowData.isHeadPerson) || false);
        ...
        onSubmit: async (values, helpers) => {
            ...
            try {
                ...
                if (response.status === 200) {
                    ...
                    const res = await findPersonBySyndicateIdApi(syndicateId);
                    dispatch({
    ...
    useEffect(() => {
        if (rowData) {
            formik.setValues({
                ...
                isLeader: isLeaderSwitch,
            });
        }
    }, [rowData, isLeaderSwitch]);
                    ...
                    <Grid item xs={12} md={12}>
                        ...
                        <FormControlLabel
                            control={
                                <Switch
                                    name="isLeader"
                                    checked={isLeaderSwitch}
                                    onChange={() => setIsLeaderSwitch(!isLeaderSwitch)}
                                />}
