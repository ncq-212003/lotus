...
import { listProcessApi, updateProcessApi, findProcessById } from "src/contexts/api/schedule/api-process";
...
export default function ProcessTable() {
  ...
  const handleCloseEditProcess = (isEvent) => {
    if (isEvent) {
  ...
  const handleDelete = async (row) => {
    try {
      ...
      dispatch({
        type: HANDLERS_PROCESS.UPDATE_PROCESS,
        payload: dataRowDelete,
      });

      const response = await updateProcessApi(dataRowDelete);
  ...
  const columns = [
    ...
    { field: "processTitle", headerName: "Tiêu đề", width: 150 },
    {
      field: "processNumber",
      headerName: "Tiến độ",
      width: 100,
    ...
    { field: "description", headerName: "Ghi chú", width: 150 },
    {
      field: "action",
      headerName: "Thao tác",
      width: 150,
  ...
  useEffect(() => {
    ...
    const updatedDataWithSTT = Array.isArray(processes) ? processes.map((pro, index) => ({
      ...pro,
      stt: index + 1,
      id: pro.id || index + 1,
    })) : [];
  ...
  const handleSearch = async () => {
    if (searchValue.length == 0) {
      ...
      const result = await findProcessById(searchValue);
  ...
  return (
    ...
    >
      ...
      <EditFormProcess
        ...
        id={selectedRow ? selectedRow.processId : ""}
