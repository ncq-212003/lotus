...
import { useStoreIntern, setFieldInput } from "src/contexts/intern-context";
...
export default function TabInfoBasic() {
  ...
  const handlePortraitPhotoChange = (event) => {
    ...
    if (file) {
      ...
      reader.onload = (e) => {
        dispatch(setFieldInput(tab, "portraitPhoto", file));
  ...
  const handleFullBodyPhotoChange = (event) => {
    ...
    if (file) {
      const reader = new FileReader();
      reader.onload = (e) => {
        dispatch(setFieldInput(tab, "fullBodyPhoto", file));
