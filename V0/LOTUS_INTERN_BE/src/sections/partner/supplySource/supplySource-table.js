...
export default function TableSupply() {
  ...
  const handleDelete = (index) => {
    ...
    const deletedContact = updatedContacts.splice(index, 1);

    if (window.confirm(`Bạn có chắc chắn muốn xóa ${deletedContact[0].name} không?`)) {
      console.log("Xóa thông tin của:", deletedContact[0]);
    }
  };
