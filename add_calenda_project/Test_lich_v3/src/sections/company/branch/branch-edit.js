...
export default function BranchEdit({ open, onClose, id }) {
  ...
  const branchEdit = Array.isArray(branchs[0]) ? branchs[0].find((bra) => bra.branchId == id) : [];
  const companyName = companyNameOption.find(
    (com) => com.value === branchEdit?.companyId
  );

  const employeeMainName = employeeNameMain.find(
    (em) => em.value === branchEdit?.employeeId
  );
    ...
    onSubmit: async (values, helpers) => {
      try {
        const formData = {
          ...
          employeeIdMain: values.mainPersonCharge,
          employeeIdMainHidden: values.mainPersonCharge,
          description: "1",
          field1: "1",
          field2: "1",
          field3: "1",
