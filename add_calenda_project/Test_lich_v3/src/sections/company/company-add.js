import { Autocomplete, Button, FormControl, Grid, IconButton, InputLabel, MenuItem, Select, Stack, TextField, Tooltip, styled } from "@mui/material";
...
import { addCompanyApi } from "src/contexts/api/company/api-company";
        ...
        onSubmit: async (values, helpers) => {
            try {
                const formData = {
                    // ...values,
                    CompanyId: "1",
                    CompanyName: values.tenCTy,
                    BriefCode: values.maCty,
                    Address: values.diaChi,
                    Email: values.email,
                    Telephone: values.sdt,
                    Website: values.website,
                    TaxCode: values.masothue,
                    TypeCompany: values.loaiCty,
                    LicenseBusiness: selectedFileGiayKinhDoanh?.name || "1",
                    Logo: selectedFileLogo?.name || "1",
                    PersonResponsibilty: values.nguoiDaiDien,
                    PersonResponsibiltyRole: values.chucVu,
                    CompanyBranchId: values.chiNhanh,
                    CompanyBranchIdHidden: values.chiNhanh,
                    EstablishCompanyDate: format(new Date(values.ngayThanhLap), 'yyyy/MM/dd'),
                    Fanpage: values.pageCty,
                    Status: values.trangThai,
                    Description: values.ghiChu || "1",
                    Field1: "1",
                    Field2: "1",
                    Field3: "1",
                    Field4: "1",
                ...
                // call api
                const response = await addCompanyApi(formData);
