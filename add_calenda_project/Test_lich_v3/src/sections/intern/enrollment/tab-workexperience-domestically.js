...
export default function TabWorkExperienceDomestically() {
  const tab = "workExperienceDomestical";
  const [state, dispatch] = useApp();
  const { intern } = state;
  const { workExperienceDomestical } = intern;
  ...
  const deleteRow = (index) => {
    dispatch(deleteRowTableIntern("workExperienceDomestical", index));
  };
  ...
  const renderDialogContent = (index) => {
    const row = workExperienceDomestical[index];
          ...
          <Grid item sm={12} md={12} xs={12}>
            ...
            <TableContainer component={Paper}>
              <Table>
                ...
                <TableBody>
                  {workExperienceDomestical.map((row, index) => (
                    <TableRow key={index}>
