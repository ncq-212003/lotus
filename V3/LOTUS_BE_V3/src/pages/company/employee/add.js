...
const Page = () => {
  ...
  const { employees, basicInfo, healthCondition, accessSystem, generalNotes } = employee;
  ...
  const handleAdd = () => {
    ...
    try {
      if (
        ...
        dispatch({
          type: HANDLERS_EMPLOYEE.ADD_EMPLOYEE,
          payload: { basicInfo, healthCondition, accessSystem, generalNotes },
        });
        setSnackbarSeverity("success");
        setSnackbarMessage("Dữ liệu đã được gửi thành công.");
        setSnackbarOpen(true);
        const response = addEmployeeApi(employees);
