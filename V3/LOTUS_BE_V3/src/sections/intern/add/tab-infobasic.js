  ...
  const validationSchema = Yup.object().shape({
    ...
    ngayHetHanHoChieu: Yup.date()
      ...
      .test("is-greater", "Ngày hết hạn phải lớn hơn ngày cấp", function (value) {
        ...
        console.log(this.resolve(Yup.ref("ngayCapHoChieu")));
...
export default function TabInfoBasic() {
  ...
  useEffect(() => {
    const getRandom = async () => {
      const res = await GenerateApi("PR-TTS", "Number");
  ...
  useEffect(() => {
    const getRandom = async () => {
      const res = await GenerateApi("TTS", "Number");
      const fieldName = "maThucTapSinh";
      const newValue = res.data;
      dispatch({
        type: HANDLERS_INTERN.SET_INPUT_INTERN,
        payload: { tab, fieldName, newValue },
