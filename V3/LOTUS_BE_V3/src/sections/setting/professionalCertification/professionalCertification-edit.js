...
import { updateCertificationApi, listCertificationApi } from "src/contexts/api/setting/api-certification";
...
export default function CertificationEdit({ open, onClose, id }) {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values, helpers) => {
            try {
                ...
                const response = await updateCertificationApi(formData);
    ...
    const selectRowEidt = Array.isArray(certifications) ? certifications.find(cer => cer.certificationCompanyId == id) : [];
